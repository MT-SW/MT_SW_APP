...
class PacketHandlerImpl(
    ...
    // Independent, out-of-band response correlation maps — unrelated to the queue-admission system above.
    ...
    // Independent response paths, unrelated to the queue-admission system above — correlate by packet/node ID only.
    ...
    override suspend fun sendToRadioAndAwaitRoutingAck(packet: MeshPacket): Boolean {
        ...
        val accepted = sendToRadio(packet)
        if (!accepted) {
